...
#ifndef REVERSE_KRANKS_BATCHRETRIEVALMERGERANKBYINTERVAL_HPP
#define REVERSE_KRANKS_BATCHRETRIEVALMERGERANKBYINTERVAL_HPP
...
namespace ReverseMIPS {
    ...
    std::unique_ptr<BaseIndex> BuildIndex(
        ...
        if (method_name != "SSMergeRankByIntervalBatchRun") {
        ...
        //disk index
        MergeRankByInterval disk_ins(user, n_data_item, disk_index_path, n_merge_user);
        disk_ins.LoadMemoryIndex(disk_memory_index_path);

        index = std::make_unique<SSMergeRankByInterval::Index>(
                //score search
                rank_bound_ins,
                //disk index
                disk_ins,
...
#endif //REVERSE_KRANKS_BATCHRETRIEVALMERGERANKBYINTERVAL_HPP
