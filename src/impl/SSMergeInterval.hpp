...
#include "alg/DiskIndex/MergeInterval.hpp"
...
namespace ReverseMIPS::SSMergeInterval {
    ...
    public:
        ...
        std::string BuildIndexStatistics() override {
            ...
            const uint64_t index_size_mb = n_merge_user * n_data_item_ * 2 * sizeof(unsigned char) / 1024 / 1024;
            char str[256];
    ...
    std::unique_ptr<Index> BuildIndex(VectorMatrix &data_item, VectorMatrix &user, const char *index_path,
        ...
        const uint64_t predict_index_size_kb = (sizeof(unsigned char) * 2) * n_data_item * n_user;
        const uint64_t n_merge_user_big_size = index_size_kb / (sizeof(unsigned char) * 2) / n_data_item;
        ...
        MergeInterval disk_ins(exact_rank_ins, user, index_path, n_data_item, n_merge_user);
