//
// Created by BianZheng on 2022/12/7.
//

#ifndef REVERSE_K_RANKS_QUERYRANKSAMPLEUNIFORMINTLRESTIMATE_HPP
#define REVERSE_K_RANKS_QUERYRANKSAMPLEUNIFORMINTLRESTIMATE_HPP
...
namespace ReverseMIPS::QueryRankSampleUniformIntLREstimate {
    ...
    class Index : public BaseIndex {
        void ResetTimer() {
            ...
            total_predict_io_cost_ = 0;
        }

        // IP Bound
        FullInt ip_bound_ins_;
        //rank bound search
        UniformLinearRegression rank_bound_ins_;
        //rank search
        ...
        uint64_t total_ip_cost_, total_io_cost_, total_refine_user_, total_predict_io_cost_;
    ...
    public:
        ...
        Index(
                ...
                UniformLinearRegression &rank_bound_ins,
                //rank search
        ...
        Retrieval(const VectorMatrix &query_item, const int &topk, const int &n_execute_query,
            ...
            for (int queryID = 0; queryID < n_query_item; queryID++) {
                ...
                if (refine_user_size <= n_user_ / 4) {
                    ...
                    total_predict_io_cost_ += pred_io_cost;
                }
        ...
        PerformanceStatistics(const int &topk) override {
            ...
            sprintf(buff,
                    "top%d retrieval time: total %.3fs, total predict %.3fs\n\tinner product %.3fs, rank bound %.3fs, prune user %.3fs, read disk %.3fs, compute rank %.3fs\n\ttotal ip cost %ld, total predict io cost %ld, total refine io cost %ld, total refine user %ld, IP bound prune ratio %.4f, rank prune ratio %.4f",
                    topk, total_retrieval_time_,
                    total_retrieval_time_ + (double) total_predict_io_cost_ / total_io_cost_ * read_disk_time_,
                    inner_product_time_, rank_bound_time_, prune_user_time_, read_disk_time_, compute_rank_time_,
                    total_ip_cost_, total_predict_io_cost_, total_io_cost_, total_refine_user_, ip_bound_prune_ratio_,
                    rank_prune_ratio_);
            std::string str(buff);
            return str;
        }
...
#endif //REVERSE_K_RANKS_QUERYRANKSAMPLEUNIFORMINTLRESTIMATE_HPP
