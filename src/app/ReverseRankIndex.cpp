...
void LoadOptions(int argc, char **argv, Parameter &para) {
    ...
    opts.add_options()
            ...
            ("method_name, mn", po::value<std::string>(&para.method_name), "method_name")
...
int main(int argc, char **argv) {
    ...
    if (method_name == "BatchDiskBruteForce") {
        ...
        sprintf(parameter_name, "cache_bound_every_%d-n_interval_%d-n_merge_user_%d", cache_bound_every, n_interval, n_merge_user);
...
//        WriteRankResult(result_rank_l[i], dataset_name, method_name.c_str());
    }
    config.AddPreprocessInfo(build_index_time);
    config.WritePerformance(dataset_name, method_name.c_str(), parameter_name);
//    config.WritePerformance(dataset_name, method_name.c_str());
    return 0;
}