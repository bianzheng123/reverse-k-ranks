...
#include "RankSample/RBNoEarlyTermination.hpp"
...
int main(int argc, char **argv) {
    ...
    const char *method_name = "RBNoEarlyTermination";
    spdlog::info("{} dataset_name {}, basic_dir {} cache_bound_every {}",
                 method_name, dataset_name, basic_dir, cache_bound_every);
    ...
    TimeRecord record;
    record.reset();
    RBNoEarlyTermination::Index &rbnet = RBNoEarlyTermination::BuildIndex(data_item, user, index_path,
                                                                          cache_bound_every);
...
//    vector<int> topk_l{20};
    RBNoEarlyTermination::RetrievalResult config;
    vector<vector<vector<UserRankElement>>> result_rank_l;
    for (int topk: topk_l) {
        record.reset();
        vector<vector<UserRankElement>> result_rk = rbnet.Retrieval(query_item, topk);

        double retrieval_time = record.get_elapsed_time_second();
        double read_disk_time = rbnet.read_disk_time_;
        double inner_product_time = rbnet.inner_product_time_;
        double coarse_binary_search_time = rbnet.coarse_binary_search_time_;
        double rank_prune_time = rbnet.rank_prune_time_;
        double fine_binary_search_time = rbnet.fine_binary_search_time_;
        double prune_ratio = rbnet.rank_prune_ratio_;
