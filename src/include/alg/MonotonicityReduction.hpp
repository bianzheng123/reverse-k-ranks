...
namespace ReverseMIPS::MonotonicityReduction {
    ...
    std::vector<double> &ConvertQuery(const double *item_vecs, const std::vector<double> &eigen_l, const int vec_dim) {
        int new_vec_dim = vec_dim + 2;
        static std::vector<double> new_item_l(new_vec_dim);

        double norm = 0;
        for (int dim = 0; dim < vec_dim; dim++) {
            norm += item_vecs[dim] * item_vecs[dim];
        }
        norm = std::sqrt(norm);
        new_item_l[1] = 0;

        for (int dim = 2; dim < new_vec_dim; dim++) {
            new_item_l[dim] = 2 * (item_vecs[dim - 2] / norm + topk_c[dim - 2]);
        }
    ...
    void ConvertUserItem(VectorMatrix &user, VectorMatrix &data_item, const std::vector<double> &eigen_l) {
        ...
        //top-k conversion
        auto topk_global_c = DBL_MAX;
        topk_b2 = 0;
        for (int userID = 0; userID < n_user; userID++) {
            double norm = 0;
            double *user_vec = user.getVector(userID);
            for (int dim = 0; dim < vec_dim; dim++) {
                norm += user_vec[dim] * user_vec[dim];
                double abs_val = std::abs(user_vec[dim]);
        ...
        for (int dim = 0; dim < vec_dim; dim++) {
            topk_c[dim] = eigen_l[dim] / eigen_total + topk_global_c;
        }

        int new_vec_dim = vec_dim + 2;
        std::unique_ptr<double[]> new_user_rawdata = std::make_unique<double[]>(n_user * (vec_dim + 2));
        std::unique_ptr<double[]> new_data_item_rawdata = std::make_unique<double[]>(n_data_item * (vec_dim + 2));

        for (int userID = 0; userID < n_user; userID++) {
            double *new_user_vecs = new_user_rawdata.get() + userID * new_vec_dim;
            double *user_vecs = user.getVector(userID);
            //compute norm for each user
            double norm = 0;
            for (int dim = 0; dim < vec_dim; dim++) {
                norm += user_vecs[dim] * user_vecs[dim];
            }
            new_user_vecs[1] = std::sqrt(topk_b2 - norm);

            for (int dim = 2; dim < new_vec_dim; dim++) {
                new_user_vecs[dim] = user_vecs[dim - 2] + topk_c[dim - 2];
            }

            norm = 0;
            for (int dim = 1; dim < new_vec_dim; dim++) {
                norm += new_user_vecs[dim] * new_user_vecs[dim];
            }
            new_user_vecs[0] = norm;
        }
        ...
        for (int itemID = 0; itemID < n_user; itemID++) {
            ...
            std::vector<double> convert_item = ConvertQuery(item_vecs, eigen_l, vec_dim);

            std::memcpy(new_item_vecs, convert_item.data(), new_vec_dim * sizeof(double));
        }
