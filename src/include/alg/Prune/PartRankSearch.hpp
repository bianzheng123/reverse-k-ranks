//
// Created by BianZheng on 2022/5/18.
//

#ifndef REVERSE_K_RANKS_PARTRANKSEARCH_HPP
#define REVERSE_K_RANKS_PARTRANKSEARCH_HPP
...
namespace ReverseMIPS {
    ...
    public:
        ...
        inline PartRankSearch() {}

        inline PartRankSearch(const int &cache_bound_every, const int &n_sample, const int &n_data_item,
                              const int &n_user) {
            if (cache_bound_every * n_sample > n_data_item) {
                spdlog::error("the sample number is larger than the number of user, program exit");
                exit(-1);
            }
            this->n_cache_rank_ = n_sample;
        ...
        void Preprocess() {
            for (int known_rank_idx = cache_bound_every_ - 1, idx = 0;
                 idx < n_cache_rank_; known_rank_idx += cache_bound_every_, idx++) {
                known_rank_idx_l_[idx] = known_rank_idx;
                assert(known_rank_idx < n_data_item_);
            }
        ...
        CoarseBinarySearch(const double &queryIP, const int &userID, const int &global_lower_bucket,
            ...
            if(bucket_ub >= n_cache_rank_){
                return false;
            }

            bucket_lb = bucket_lb >= n_cache_rank_ ? n_cache_rank_ - 1 : bucket_lb;

            assert(0 <= bucket_ub && bucket_ub <= bucket_lb && bucket_lb < n_cache_rank_);
...
#endif //REVERSE_K_RANKS_PARTRANKSEARCH_HPP
