...
#pragma omp parallel for default(none) shared(data, means, distances)
...
*/
        ...
        random_plusplus_parallel(const std::vector<std::vector<T>> &data, uint32_t k, uint64_t seed) {
            assert(k > 0);
            assert(data.size() > 0);
...
#pragma omp parallel for default(none) shared(data, means, clusters)
            for (size_t i = 0; i < data.size(); ++i) {
...
*/
    ...
    std::tuple<std::vector<std::vector<T>>, std::vector<uint32_t>> kmeans_lloyd_parallel(
        ...
        std::vector<std::vector<T>> means = details::random_plusplus_parallel(data, parameters.get_k(), seed);
