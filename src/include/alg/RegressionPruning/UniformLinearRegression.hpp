...
#pragma omp parallel for default(none) shared(sampleIP_l, low_score, score_diff)
            for (int sampleID = 0; sampleID < n_sample_rank_ - 1; sampleID++) {
                const double input_next_x = (sampleIP_l[sampleID + 1] - low_score) / score_diff;
                preprocess_cache_A_[(4 * sampleID) * (n_predict_parameter_ + 1)] = -input_next_x;
                preprocess_cache_A_[(4 * sampleID) * (n_predict_parameter_ + 1) + 1] = -1;
                preprocess_cache_A_[(4 * sampleID) * (n_predict_parameter_ + 1) + 2] = -1;

                preprocess_cache_A_[(4 * sampleID) * (n_predict_parameter_ + 1) + 3] = input_next_x;
                ...
                preprocess_cache_A_[(4 * sampleID) * (n_predict_parameter_ + 1) + 6] = -input_x;
                preprocess_cache_A_[(4 * sampleID) * (n_predict_parameter_ + 1) + 7] = -1;
                preprocess_cache_A_[(4 * sampleID) * (n_predict_parameter_ + 1) + 8] = -1;

                preprocess_cache_A_[(4 * sampleID) * (n_predict_parameter_ + 1) + 9] = input_x;
...

            {
                ...
                const double input_x = (sampleIP_l[sampleID] - low_score) / score_diff;
                preprocess_cache_A_[(4 * (n_sample_rank_ - 1)) * (n_predict_parameter_ + 1) + 0] = -input_x;
                ...
                preprocess_cache_A_[(4 * (n_sample_rank_ - 1)) * (n_predict_parameter_ + 1) + 3] = input_x;
...

            {
                ...
                const double input_x = (sampleIP_l[sampleID] - low_score) / score_diff;
                preprocess_cache_A_[(4 * (n_sample_rank_ - 1)) * (n_predict_parameter_ + 1) + 6] = -input_x;
                ...
                preprocess_cache_A_[(4 * (n_sample_rank_ - 1)) * (n_predict_parameter_ + 1) + 9] = input_x;
...
//            printf("res [0]: %.3f, [1]: %.3f\n", res[0], res[1]);
            ...
            //assign error
            double error = -1;
#pragma omp parallel for default(none) shared(userID, error, sampleIP_l, low_score, score_diff)
            for (int sampleID = 0; sampleID < n_sample_rank_ - 1; sampleID++) {
                const double input_x = (sampleIP_l[sampleID] - low_score) / score_diff;
                const double input_next_x = (sampleIP_l[sampleID + 1] - low_score) / score_diff;
                ...
                const double pred_rank1 = input_next_x * tmp_a + tmp_b;
                const double pred_rank2 = input_x * tmp_a + tmp_b;
