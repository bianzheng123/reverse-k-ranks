...
int main(int argc, char **argv) {
    ...
    spdlog::info("IRBPartDimPartNormPrune dataset_name {}, basic_dir {}", dataset_name, basic_dir);
    ...
    for (int i = 0; i < n_topk; i++) {
        ...
        writeRank(result_rank_l[i], dataset_name, "IRBPartDimPartNormPrune");
    }

    config.AddPreprocess(build_index_time);
    config.writePerformance(dataset_name, "IRBPartDimPartNormPrune");
    return 0;
}