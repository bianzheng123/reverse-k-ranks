...
int main(int argc, char **argv) {
    ...
    spdlog::info("RankBound dataset_name {}, basic_dir {}", dataset_name, basic_dir);
    ...
    TimeRecord record;
    record.reset();
    RankBound::Index &bscb = RankBound::BuildIndex(data_item, user, index_path);
    double build_index_time = record.get_elapsed_time_second();
    spdlog::info("finish preprocess and save the index");

    vector<int> topk_l{10, 20, 30, 40, 50};
    vector<RankBound::RetrievalResult> retrieval_res_l;
    ...
    writePerformance(dataset_name, "RankBound", performance_m);
