...
int main(int argc, char **argv) {
    ...
    spdlog::info("BPlusTree dataset_name {}, basic_dir {}", dataset_name, basic_dir);
...
//    vector<int> topk_l{10};
    ...
    for (int i = 0; i < n_topk; i++) {
        ...
        writeRank(result_rank_l[i], dataset_name, "BPlusTree");
    }

    config.AddPreprocess(total_build_index_time);
    config.writePerformance(dataset_name, "BPlusTree");
