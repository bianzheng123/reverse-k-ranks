...
#ifndef REVERSE_KRANKS_EXACTUSERRANK_HPP
...
namespace ReverseMIPS::ExactUserRank {
...
//            spdlog::info("index size %ld byte\n", fsize);
    ...
    public:
        ...
        std::unique_ptr<int[]> GetAllRank(const VectorMatrix &query_item) {
            ResetTimer();
            ...
            std::unique_ptr<int[]> query_rank_l = std::make_unique<int[]>(n_query_item * n_user_);
            ...
            for (int qID = 0; qID < n_query_item; qID++) {
                ...
                for (int userID = 0; userID < n_user_; userID++) {
                    int user_offset = qID * n_user_ + userID;
                    query_rank_l[user_offset] = user_rank_l[userID];
                }

            }

            tree_ins_.FinishRetrieval();
            return std::move(query_rank_l);
        }
...
#endif //REVERSE_KRANKS_EXACTUSERRANK_HPP
