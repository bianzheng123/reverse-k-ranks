...
namespace ReverseMIPS::RankSamplePrintPruneRatio {
    ...
    public:
        ...
        void Retrieval(const VectorMatrix &query_item, const int &topk, double *prune_ratio_l) {
            ResetTimer();
    ...
    void MeasurePruneRatio(const char *dataset_name, const char *basic_dir, const char* memeory_index_path, const int &n_sample) {
        //measure prune ratio
        ...
        std::unique_ptr<RankSamplePrintPruneRatio::Index> index =
                RankSamplePrintPruneRatio::BuildIndex(data_item, user, memeory_index_path);

        std::string method_name = "RankSamplePrintPruneRatio";
        char parameter_name[256];
        sprintf(parameter_name, "n_sample_%d", n_sample);

        PrintPruneRatio::RetrievalResultAttribution config;
        std::vector<int> topk_l = {10};
        TimeRecord record;
        for (const int topk: topk_l) {
            record.reset();
            std::vector<double> prune_ratio_l(n_query_item);
            index->Retrieval(query_item, topk, prune_ratio_l.data());
            ...
            config.WritePruneRatio(prune_ratio_l, n_query_item, topk,
