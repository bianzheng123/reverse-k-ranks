...
ScoreDistribution(const VectorMatrix &user, const VectorMatrix &data_item, const int n_sample_user) {
    ...
    static std::vector<std::vector<double>> score_dist_l(n_sample_user, std::vector<double>(n_data_item));
...
void WriteRank(const std::vector<std::vector<double>> &score_list_l, const char *dataset_name) {
    ...
    for (int sampleID = 0; sampleID < n_sample_user; sampleID++) {
        const std::vector<double> user_distribution = score_list_l[sampleID];
...
int main(int argc, char **argv) {
    ...
    std::vector<std::vector<double>> score_list = ScoreDistribution(user, data_item, n_sample_user);
