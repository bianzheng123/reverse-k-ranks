...
for b_est in [4, 4.5, 5.5, 6.0, 6.5, 7.0]:
    a_est = 1.7
    ...
    est_x = np.array([1 - np.exp(-((_ / a_est) ** b_est)) for _ in x_process])
    ...
    ax.set_ylabel('Rank')
    if is_test:
        plt.savefig("ScoreRankCurve_fit_weibull_b.jpg".format(method_name), bbox_inches='tight', dpi=600)
    else:
        plt.savefig("ScoreRankCurve_fit_weibull_b.pdf".format(method_name), bbox_inches='tight')

fig = plt.figure(figsize=(6, 4))
subplot_str = 111
ax = fig.add_subplot(subplot_str)
ax.plot(score_l, rank_l, color='#ff0000', linestyle='solid', linewidth=3, label='Score')
for a_est in [1.7, 1.75, 1.8, 1.85, 1.9, 1.95]:
    b_est = 5.5
    # a_est = np.power(1 / len(score_l) * np.sum([_ ** b_est for _ in score_l]), 1 / b_est)
    est_x = np.array([1 - np.exp(-((_ / a_est) ** b_est)) for _ in x_process])
    est_x = - len(rank_l) * est_x + len(rank_l)
    ...
    ax.set_ylabel('Rank')
    if is_test:
        plt.savefig("ScoreRankCurve_fit_weibull_a.jpg".format(method_name), bbox_inches='tight', dpi=600)
    else:
        plt.savefig("ScoreRankCurve_fit_weibull_a.pdf".format(method_name), bbox_inches='tight')
