//
// Created by BianZheng on 2022/5/31.
//

#ifndef REVERSE_K_RANKS_ITEMPQ_HPP
#define REVERSE_K_RANKS_ITEMPQ_HPP
...
namespace ReverseMIPS {
    ...
    class CAItemPQ : public BaseIPBound {
        int n_user_, n_data_item_, vec_dim_, n_codeword_, n_codebook_;
        int normal_dim_, last_dim_;
        ...
        std::unique_ptr<double[]> user_norm_l_;//n_user, store exact user norm

        //retrieval
        std::vector<bool> is_compute_l_;//n_codebook * n_codeword, record whether it is computed
        std::unique_ptr<double[]> appr_ip_l_;//n_codebook * n_codeword, store whether it is computed
    ...
    public:
        ...
        inline CAItemPQ(const int &n_user, const int &n_data_item, const int &vec_dim,
            ...
            this->is_compute_l_.resize(n_codebook * n_codeword);
            this->appr_ip_l_ = std::make_unique<double[]>(n_codebook * n_codeword);
            ...
            spdlog::info("n_codebook {}, n_codeword {}, normal_dim {}, last_dim {}",
                         n_codebook_, n_codeword_, normal_dim_, last_dim_);
        }

        void BuildCode(const VectorMatrix &vm,
                       double *vm_codebook, unsigned int *vm_codeword, double *vecs_error_l) {
        ...
        void Preprocess(VectorMatrix &user, VectorMatrix &data_item) {
            ...
            BuildCode(data_item,
                      item_codebook_.get(), item_codeword_.get(), item_error_l_.get());

            for (int userID = 0; userID < n_user_; userID++) {
                const double *user_vecs = user.getVector(userID);
                double IP = InnerProduct(user_vecs, user_vecs, vec_dim_);
        ...
        std::pair<double, double> BoundData(const double *user_vecs, const int &userID,
                                            const double *item_vecs, const int &itemID) {
            double appr_IP = 0;
            for (int cbookID = 0; cbookID < n_codebook_; cbookID++) {
                const int offset_dim = cbookID * normal_dim_;
                const int part_dim = cbookID == n_codebook_ - 1 ? last_dim_ : normal_dim_;
                unsigned int citemID = item_codeword_[itemID * n_codebook_ + cbookID];
                assert(0 <= citemID && citemID < n_codeword_);

                const double *part_item_vecs = item_codebook_.get() + citemID * vec_dim_ + offset_dim;
                double partIP = InnerProduct(part_item_vecs, user_vecs + offset_dim, part_dim);
                appr_IP += partIP;
            }

            double remain_bound = item_error_l_[itemID] * user_norm_l_[userID];

            return std::make_pair(appr_IP, remain_bound);
        }

        double IPUpperBound(const double *user_vecs, const int &userID,
                            const double *item_vecs, const int &itemID) override {
            std::pair<double, double> bound_data = BoundData(user_vecs, userID, item_vecs, itemID);
            double IP_ub = bound_data.first + bound_data.second;
        ...
        double IPLowerBound(const double *user_vecs, const int &userID,
                            const double *item_vecs, const int &itemID) override {
            std::pair<double, double> bound_data = BoundData(user_vecs, userID, item_vecs, itemID);
            double IP_lb = bound_data.first - bound_data.second;
        ...
        IPBound(const double *user_vecs, const int &userID,
            ...
            is_compute_l_.assign(n_codebook_ * n_codeword_, false);
            ...
            for (const int &itemID: item_cand_l) {
                ...
                for (int cbookID = 0; cbookID < n_codebook_; cbookID++) {
                    ...
                    double partIP;
                    if (is_compute_l_[cbookID * n_codeword_ + citemID]) {
                        partIP = appr_ip_l_[cbookID * n_codeword_ + citemID];
                    } else {
                        const int offset_dim = cbookID * normal_dim_;
                        const int part_dim = cbookID == n_codebook_ - 1 ? last_dim_ : normal_dim_;

                        const double *part_item_vecs = item_codebook_.get() + citemID * vec_dim_ + offset_dim;
                        double IP = InnerProduct(part_item_vecs, user_vecs + offset_dim, part_dim);
                        appr_ip_l_[cbookID * n_codeword_ + citemID] = IP;
                        partIP = IP;
                        is_compute_l_[cbookID * n_codeword_ + citemID] = true;
                    }
                ...
                double remain_bound = item_error_l_[itemID] * user_norm_l_[userID];
                double IP_lb = appr_IP - remain_bound;
                double IP_ub = appr_IP + remain_bound;
                IPbound_l[itemID] = std::pair<double, double>(IP_lb, IP_ub);
            }
...
#endif //REVERSE_K_RANKS_ITEMPQ_HPP
