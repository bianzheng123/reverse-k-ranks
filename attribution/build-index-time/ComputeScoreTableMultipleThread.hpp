...
#ifndef REVERSE_KRANKS_COMPUTESCORETABLEMULTIPLETHREAD_HPP
#define REVERSE_KRANKS_COMPUTESCORETABLEMULTIPLETHREAD_HPP
...
#include "CPUScoreTableMultipleThread.hpp"


namespace ReverseMIPS {
    class ComputeScoreTableMultipleThread {
        TimeRecord record_;
        int n_data_item_;
        std::vector<double> ip_cache_l_;

        CPUScoreTableMultipleThread cpu;
    public:
        const int report_every_ = 30000;

        double compute_time_, sort_time_;

        ComputeScoreTableMultipleThread() = default;

        inline ComputeScoreTableMultipleThread(const VectorMatrix &user, const VectorMatrix &data_item) {
            ...
            cpu = CPUScoreTableMultipleThread(user_vecs, item_vecs, n_user, n_data_item, vec_dim);
        }
...
#endif //REVERSE_KRANKS_COMPUTESCORETABLEMULTIPLETHREAD_HPP
