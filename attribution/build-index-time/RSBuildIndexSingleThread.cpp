...
int main(int argc, char **argv) {
    ...
    TimeRecord record;
    record.reset();
    BuildRankSample::BuildIndexSingleThread(data_item, user, index_path,
    ...
    sprintf(parameter_name, "n_sample_%d-index_size_gb_%lu-single_thread", n_sample, index_size_gb);

    spdlog::info("finish preprocess and save the index");

    RetrievalResult2 config;

    spdlog::info("build index time: single thread {}s, compute time {}s, sort time {}s, process index time {}s",
                 build_index_time, total_compute_time, total_sort_time, process_index_time);

    config.AddBuildIndexTime("Single Thread", build_index_time);
